...
void pdlBlinkThread(void *arg)
{
    ...
    WPRINT_APP_INFO(("Started blink thread\r\n"));
     while(1)
     {
         Cy_GPIO_Inv(GPIO_PRT0,3);
         //sprintf(buff,"Val = %d\r\n",Cy_GPIO_ReadOut(GPIO_PRT0,3));
         //Cy_SCB_UART_PutString(SCB5,"Blink\r\n");
...
void application_start( )
{
    ...
    //wiced_hal_blink();
    wiced_result_t result=wiced_rtos_create_thread(&blinkThreadHandle,8,"Blink Thread",pdlBlinkThread,500,0);
    WPRINT_APP_INFO(("thread start=%d\r\n",result));

    wiced_rtos_create_thread(&capsenseThreadHandle,8,"CapSense Thread",capsenseThread,4096,0);
