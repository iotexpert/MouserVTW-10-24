 ...
 * Function Prototypes
 ******************************************************************/
static void                   remote719_app_init               ( void );
static wiced_bt_dev_status_t  remote719_management_callback    ( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data );
static void                   remote719_set_advertisement_data ( void );
static void                   remote719_advertisement_stopped  ( void );
static void                   remote719_reset_device           ( void );
/* GATT Registration Callbacks */
static wiced_bt_gatt_status_t remote719_write_handler          ( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id );
static wiced_bt_gatt_status_t remote719_read_handler           ( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id );
static wiced_bt_gatt_status_t remote719_connect_callback       ( wiced_bt_gatt_connection_status_t *p_conn_status );
static wiced_bt_gatt_status_t remote719_server_callback        ( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data );
static wiced_bt_gatt_status_t remote719_event_handler          ( wiced_bt_gatt_evt_t  event, wiced_bt_gatt_event_data_t *p_event_data );
#if 0
static uint32_t               hci_control_process_rx_cmd       ( uint8_t* p_data, uint32_t len );
static void                   remote719_trace_callback         ( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );
#endif
 ...
 ******************************************************************/
uint8_t remote719_generic_access_device_name[] = {'r','e','m','o','t','e','7','1','9'};
uint8_t remote719_generic_access_appearance[]  = {0x00,0x00};
uint8_t remote719_remote_paddle[]              = {'0'};
...
gatt_db_lookup_table remote719_gatt_db_ext_attr_tbl[] =
{
    /* { attribute handle,                  maxlen,                          curlen,                          attribute data } */
    {HDLC_GENERIC_ACCESS_DEVICE_NAME_VALUE, 9,                               9,                               remote719_generic_access_device_name},
    {HDLC_GENERIC_ACCESS_APPEARANCE_VALUE,  2,                               2,                               remote719_generic_access_appearance},
    {HDLC_REMOTE_PADDLE_VALUE,              sizeof(remote719_remote_paddle), sizeof(remote719_remote_paddle), remote719_remote_paddle},
};

// Number of Lookup Table Entries
const uint16_t remote719_gatt_db_ext_attr_tbl_size = ( sizeof ( remote719_gatt_db_ext_attr_tbl ) / sizeof ( gatt_db_lookup_table ) );
...
//    transport_pool = wiced_transport_create_buffer_pool ( TRANS_UART_BUFFER_SIZE, TRANS_UART_BUFFER_COUNT );
...
#if ((defined WPRINT_APP_INFO_ENABLE) || (defined HCI_TRACE_OVER_TRANSPORT))
    ...
    WPRINT_APP_INFO(("Starting ble stack\r\n"));
    /* Initialize Bluetooth Controller and Host Stack */
    wiced_result_t result=wiced_bt_stack_init(remote719_management_callback, &wiced_bt_cfg_settings, wiced_bt_cfg_buf_pools);

    WPRINT_APP_INFO(("Finished ble stack result=%d\r\n",result));
}

/*
 * This function is executed in the BTM_ENABLED_EVT management callback.
 */
void remote719_app_init(void)
{
  ...
  //  wiced_bt_app_init();
    ...
    remote719_set_advertisement_data();

    /* Register with stack to receive GATT callback */
    wiced_bt_gatt_register( remote719_event_handler );
    ...
    wiced_result_t result = wiced_bt_start_advertisements(BTM_BLE_ADVERT_UNDIRECTED_HIGH, 0, NULL);
    WPRINT_APP_INFO(("Start Advertisements =%d\r\n",result));

}

/* Set Advertisement Data */
void remote719_set_advertisement_data( void )
{
    ...
    wiced_result_t result = wiced_bt_ble_set_raw_advertisement_data(num_elem, adv_elem);
    WPRINT_APP_INFO(("Adver Set Data=%d\r\n",result));
}

/* This function is invoked when advertisements stop */
void remote719_advertisement_stopped( void )
{
    WPRINT_APP_INFO(("Advertisement stopped\n"));

    /* TODO: Handle when advertisements stop */
}

/* TODO: This function should be called when the device needs to be reset */
void remote719_reset_device( void )
{
...
wiced_bt_dev_status_t remote719_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        wiced_bt_dev_register_hci_trace(remote719_trace_callback);
#endif
        ...
        if (WICED_BT_SUCCESS == p_event_data->enabled.status)
        {
            ...
            WPRINT_APP_INFO(("Local Bluetooth Address: [%B]\n", bda));
    ...
    case BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT:
        ...
        {
            WPRINT_APP_INFO(("Key Retrieval Failure\n"));
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        {
            remote719_advertisement_stopped();
        }
        break;
...
/* Get a Value */
wiced_bt_gatt_status_t remote719_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    int i = 0;
    ...
    for (i = 0; i < remote719_gatt_db_ext_attr_tbl_size; i++)
    {
        if (remote719_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            // Detected a matching handle in external lookup table
            isHandleInTable = WICED_TRUE;
            // Detected a matching handle in the external lookup table
            if (remote719_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
            {
                // Value fits within the supplied buffer; copy over the value
                *p_len = remote719_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, remote719_gatt_db_ext_attr_tbl[i].p_data, remote719_gatt_db_ext_attr_tbl[i].cur_len);
    ...
    if (!isHandleInTable)
    {
        ...
        default:
            ...
            WPRINT_APP_INFO(("Read Request to Invalid Handle: 0x%x\n", attr_handle));
...
/* Set a Value */
wiced_bt_gatt_status_t remote719_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    int i = 0;
    wiced_bool_t isHandleInTable = WICED_FALSE;
    ...
    for (i = 0; i < remote719_gatt_db_ext_attr_tbl_size; i++)
    {
        if (remote719_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            validLen = (remote719_gatt_db_ext_attr_tbl[i].max_len >= len);
            if (validLen)
            {
                // Value fits within the supplied buffer; copy over the value
                remote719_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(remote719_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
    ...
    if (!isHandleInTable)
    {
        ...
        default:
            ...
            WPRINT_APP_INFO(("Write Request to Invalid Handle: 0x%x\n", attr_handle));
...
wiced_bt_gatt_status_t remote719_write_handler( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = remote719_set_value(p_write_req->handle, conn_id, p_write_req->p_val, p_write_req->val_len);

    return status;
}

/* Handles Read Requests received from Client device */
wiced_bt_gatt_status_t remote719_read_handler( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = remote719_get_value(p_read_req->handle, conn_id, p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);

    return status;
}

/* GATT Connection Status Callback */
wiced_bt_gatt_status_t remote719_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
    ...
    if ( NULL != p_conn_status )
    {
        if ( p_conn_status->connected )
        {
            ...
            WPRINT_APP_INFO(("Connected : BDA '%B', Connection ID '%d'\n", p_conn_status->bd_addr, p_conn_status->conn_id ));
...
wiced_bt_gatt_status_t remote719_server_callback( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATTS_REQ_TYPE_READ:
        status = remote719_read_handler( &p_data->read_req, conn_id );
        break;
    case GATTS_REQ_TYPE_WRITE:
        status = remote719_write_handler( &p_data->write_req, conn_id );
        break;
    }
...
/* GATT Event Handler */
wiced_bt_gatt_status_t remote719_event_handler( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_event_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = remote719_connect_callback( &p_event_data->connection_status );
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = remote719_server_callback( p_attr_req->conn_id, p_attr_req->request_type, &p_attr_req->data );
        break;
    default:
        status = WICED_BT_GATT_SUCCESS;
        break;
...
uint32_t hci_control_process_rx_cmd( uint8_t* p_data, uint32_t len )
{
    ...
    WPRINT_APP_INFO("hci_control_process_rx_cmd : Data Length '%d'\n", len);

    // At least 4 bytes are expected in WICED Header
    if ((NULL == p_data) || (len < 4))
    {
        WPRINT_APP_INFO("Invalid Parameters\n"));
...
void remote719_trace_callback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
